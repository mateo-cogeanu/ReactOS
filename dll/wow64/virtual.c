...
#else
    status = NtFlushVirtualMemory(process, (void **)addr_32to64(&addr, addr32),
                                  size_32to64(&size, size32), &iosb64);
#endif
...
#else
    void *prev = NtCurrentTeb()->NtTib.ArbitraryUserPointer;
#endif

#ifndef __REACTOS__
    NtCurrentTeb()->Tib.ArbitraryUserPointer = ULongToPtr( NtCurrentTeb32()->Tib.ArbitraryUserPointer );
#else
    NtCurrentTeb()->NtTib.ArbitraryUserPointer = ULongToPtr( NtCurrentTeb32()->NtTib.ArbitraryUserPointer );
#endif
...
#else
    NtCurrentTeb()->NtTib.ArbitraryUserPointer = prev;
#endif
    return status;
}
