...
#else
    info32->unknown                      = info->Reserved;
    info32->KeMaximumIncrement           = info->TimerResolution;
    info32->PageSize                     = info->PageSize;
    info32->MmNumberOfPhysicalPages      = info->NumberOfPhysicalPages;
    info32->MmLowestPhysicalPage         = info->LowestPhysicalPageNumber;
    info32->MmHighestPhysicalPage        = info->HighestPhysicalPageNumber;
    info32->AllocationGranularity        = info->AllocationGranularity;
    info32->LowestUserAddress            = (ULONG)info->MinimumUserModeAddress;
    info32->HighestUserAddress           = (ULONG)info->MaximumUserModeAddress;
    info32->ActiveProcessorsAffinityMask = info->ActiveProcessorsAffinityMask;
    info32->NumberOfProcessors           = info->NumberOfProcessors;
#endif
}
        ...
        if (len >= sizeof(SYSTEM_HANDLE_INFORMATION32))
        {
            ...
            if (!(status = NtQuerySystemInformation( class, info, size, retlen )))
            {
                ...
                for (i = 0; i < info->NumberOfHandles; i++)
                {
                    ...
                    info32->Handle[i].HandleFlags   = info->Handles[i].HandleAttributes;
                    info32->Handle[i].HandleValue   = info->Handles[i].HandleValue;
...
#else
                ULONG count = (*retlen - offsetof(SYSTEM_HANDLE_INFORMATION, Handles)) / sizeof(SYSTEM_HANDLE_TABLE_ENTRY_INFO);
#endif
...
#else
                info32->PagedPoolSize = info.PagedPoolSize;
#endif
            }
        }
        else status = STATUS_INFO_LENGTH_MISMATCH;
...
#else
                    info32[i].BaseInfo.Section           = mod->BaseInfo.Section;
#endif
...
#else
                    info32[i].BaseInfo.NameOffset        = mod->BaseInfo.OffsetToFileName;
#endif
...
#else
                    strcpy( (char *)info32[i].BaseInfo.Name, (char *)mod->BaseInfo.FullPathName );
#endif
...
#else
    LARGE_INTEGER *new = get_ptr( &args );
#endif
