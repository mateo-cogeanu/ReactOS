...
#else
static PCLSMENUNAME client_menu_name_32to64( PCLSMENUNAME name,
                                             const struct client_menu_name32 *name32,
                                             UNICODE_STRING* pusMenuName)
{
    if (!name32) return NULL;
    name->pszClientAnsiMenuName = UlongToPtr( name32->nameA );
    name->pwszClientUnicodeMenuName = UlongToPtr( name32->nameW );
    name->pusMenuName = unicode_str_32to64(pusMenuName, UlongToPtr(name32->nameUS));
    return name;
}

static struct client_menu_name32 *client_menu_name_64to32( const CLSMENUNAME *name64,
                                                           struct client_menu_name32 *name32 )
{
    if (name32)
    {
        struct client_menu_name32 name;
        name.nameA = PtrToUlong( name64->pszClientAnsiMenuName );
        name.nameW = PtrToUlong( name64->pwszClientUnicodeMenuName );
        name.nameUS = PtrToUlong( name64->pusMenuName );
        memcpy( name32, &name, sizeof(name) );
    }
    return name32;
}
...
#else
    const MOUSEHOOKSTRUCT* hook64cast = (const MOUSEHOOKSTRUCT*)hook64;

    hook.pt           = hook64cast->pt;
    hook.hwnd         = HandleToUlong( hook64cast->hwnd );
    hook.wHitTestCode = hook64cast->wHitTestCode;
    hook.dwExtraInfo  = hook64cast->dwExtraInfo;
    hook.mouseData    = hook64->mouseData;
    memcpy( hook32, &hook, sizeof(hook) );
#endif
    return hook32;
...
#else
    PWINDOWPROC_CALLBACK_ARGUMENTS params = arg;
...
#else
    WndProcParams64To32(params, params32);
#endif
...
#else
    LARGE_STRING32* plstrClassName32 = get_ptr(&args);
    LARGE_STRING32* plstrVersion32 = get_ptr(&args);
    LARGE_STRING32* plstrWindowName32 = get_ptr(&args);
#endif
...
#else
    PVOID pCtx = get_ptr( &args );
#endif

#ifndef __REACTOS__
    UNICODE_STRING class_name, version, window_name;
#else
    LARGE_STRING lstrClassName, lstrWindowName, lstrVersion;
#endif
    HWND ret;
...
#else
                                large_str_32to64(&lstrClassName, plstrClassName32),
                                large_str_32to64(&lstrVersion, plstrVersion32),
                                large_str_32to64(&lstrWindowName, plstrWindowName32),
#endif
                                style, x, y, width, height, parent, menu,
#ifndef __REACTOS__
                                instance, params, flags, client_instance, unk, ansi );
#else
                                instance, params, flags, pCtx);
#endif
    return HandleToUlong( ret );
}
...
NTSTATUS WINAPI wow64_NtUserRegisterClassExWOW( UINT *args )
{
    ...
    struct client_menu_name client_name;
#else
    UNICODE_STRING usMenuName;
    CLSMENUNAME client_name;
#endif
    ...
    return NtUserRegisterClassExWOW( &wc,
                                     ...
                                     unicode_str_32to64( &version, version32 ),
                                     client_menu_name_32to64( &client_name, client_name32, &usMenuName ),
                                     fnid, flags, wow );
}
...
#else
    procs->pfnScrollBarWndProc      = UlongToPtr(procs32->pfnScrollBarWndProc);
    procs->pfnTitleWndProc          = UlongToPtr(procs32->pfnTitleWndProc);
    procs->pfnMenuWndProc           = UlongToPtr(procs32->pfnMenuWndProc);
    procs->pfnDesktopWndProc        = UlongToPtr(procs32->pfnDesktopWndProc);
    procs->pfnDefWindowProc         = UlongToPtr(procs32->pfnDefWindowProc);
    procs->pfnMessageWindowProc     = UlongToPtr(procs32->pfnMessageWindowProc);
    procs->pfnSwitchWindowProc      = UlongToPtr(procs32->pfnSwitchWindowProc);
    procs->pfnButtonWndProc         = UlongToPtr(procs32->pfnButtonWndProc);
    procs->pfnComboBoxWndProc       = UlongToPtr(procs32->pfnComboBoxWndProc);
    procs->pfnComboListBoxProc      = UlongToPtr(procs32->pfnComboListBoxProc);
    procs->pfnDialogWndProc         = UlongToPtr(procs32->pfnDialogWndProc);
    procs->pfnEditWndProc           = UlongToPtr(procs32->pfnEditWndProc);
    procs->pfnListBoxWndProc        = UlongToPtr(procs32->pfnListBoxWndProc);
    procs->pfnMDIClientWndProc      = UlongToPtr(procs32->pfnMDIClientWndProc);
    procs->pfnStaticWndProc         = UlongToPtr(procs32->pfnStaticWndProc);
    procs->pfnImeWndProc            = UlongToPtr(procs32->pfnImeWndProc);
    procs->pfnGhostWndProc          = UlongToPtr(procs32->pfnGhostWndProc);
    procs->pfnHkINLPCWPSTRUCT       = UlongToPtr(procs32->pfnHkINLPCWPSTRUCT);
    procs->pfnHkINLPCWPRETSTRUCT    = UlongToPtr(procs32->pfnHkINLPCWPRETSTRUCT);
    procs->pfnDispatchHook          = UlongToPtr(procs32->pfnDispatchHook);
    procs->pfnDispatchDefWindowProc = UlongToPtr(procs32->pfnDispatchDefWindowProc);
    procs->pfnDispatchMessage       = UlongToPtr(procs32->pfnDispatchMessage);
    procs->pfnMDIActivateDlgProc    = UlongToPtr(procs32->pfnMDIActivateDlgProc);
#endif
    return procs;
}
...
NTSTATUS WINAPI wow64_NtUserMapVirtualKeyEx( UINT *args )
{
    ...
    UINT type = get_ulong( &args );
#ifdef __REACTOS__
    DWORD keyboardId = get_ulong( &args );
#endif
    HKL layout = get_handle( &args );
...
#else
    return NtUserMapVirtualKeyEx(code, type, keyboardId, layout);
#endif
}
