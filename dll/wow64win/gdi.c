...
/* FIXME */
#define NTGDI_OBJ_EXTPEN       GDI_OBJECT_TYPE_EXTPEN
#define NTGDI_OBJ_PEN          GDI_OBJECT_TYPE_PEN
#define NTGDI_OBJ_BITMAP       GDI_OBJECT_TYPE_BITMAP
#define NTGDI_HANDLE_TYPE_MASK GDI_HANDLE_TYPE_MASK
...
#else
    DWORD blend_function_ulong = get_ulong( &args );
    BLENDFUNCTION blend_function = *((BLENDFUNCTION*)&blend_function_ulong);
#endif
...
#else
    PCHWIDTHINFO info = get_ptr( &args );
#endif
...
#else
    TMW_INTERNAL *metrics = get_ptr( &args );
#endif
    ULONG flags = get_ulong( &args );
...
#else
    DWORD ptr1 = get_ulong( &args );
    DWORD data2 = get_ulong( &args );
    DWORD ptr3 = get_ulong( &args );
    DWORD data4 = get_ulong( &args );
#endif
...
#else
    DWORD magic_ulong = get_ulong( &args );
    PALETTEENTRY magic = *((PALETTEENTRY*)&magic_ulong);
#endif
