...
VOID
DumpTestMiniport(
    _Inout_ PFDO_DEVICE_EXTENSION FdoExt);
    ...
    _In_reads_(ATA_MAX_DEVICE) PCHANNEL_DEVICE_CONFIG* DeviceList)
{
    ...
    for (i = 0; i < ATA_MAX_DEVICE; ++i)
    {
        ...
        Device->SupportedModes &= ChanData->Current.TransferModeSupported;
...
VOID
AtaCtrlPciSaveData(
    _Out_ PATA_CONTROLLER Controller,
    _In_ PPCI_COMMON_HEADER PciData)
{
    PAGED_CODE();
    ...
    _In_ PCM_RESOURCE_LIST ResourcesTranslated)
{
    ...
    Controller->Flags &= CTRL_FLAG_NON_PNP;
    ...
    {
        Status = PataGetControllerProperties(Controller, ResourcesTranslated);
    }
    else
#endif
    {
    ...
    _In_ PVOID ControllerContext)
{
         ...
         ListEntry = ListEntry->Flink)
    {
        ...
        PciIdeXPdoRemoveDevice(PdoExt, NULL, TRUE);
    }
    ...
    _In_ ULONG ChannelNumber)
{
    ...
    WCHAR DeviceNameBuffer[sizeof("\\Device\\Ide\\PciIde99999Channel99-FFF")];
    PDEVICE_OBJECT Pdo;
    ...
    _In_ PIO_STACK_LOCATION IoStack)
{
    ...
    switch (IoStack->Parameters.QueryId.IdType)
    {
        ...
        {
            if (!NT_VERIFY(IoForwardIrpSynchronously(FdoExt->Common.LowerDeviceObject, Irp)))
            {
            ...
            if (Status == STATUS_NOT_SUPPORTED)
            {
                ...
                Buffer = ExAllocatePoolUninitialized(PagedPool, sizeof(IdeGenericId), TAG_PCIIDEX);
                if (!Buffer)
                {
    ...
    _Inout_ PIRP Irp)
{
    ...
    switch (IoStack->MinorFunction)
    {
        ...
        case IRP_MN_REMOVE_DEVICE:
        {
            ...
            PciIdeXFdoRemoveDevice(FdoExtension);
